...
class TransactionManager:
    ...
    """
    def __init__(self):
        self.trans_list = []    #transaction list
        self.site_list = []     #site list
        self.wait_list = [] #wait table???

    def init_sitelist(self):
        """
    ...
    def dump(self, var = -1):
        ...
        if var != -1:
            ...
            for i in len(self.site_list):
                if self.site_list[i].variable[var-1] is not None:
                    print("site{}: {};".format(i+1, self.site_list[i].variable[var-1]), end = " ")
            print()
        else:
            for i in len(self.site_list):
